package org.hammel.paintpdf.presenter
...
import org.hammel.paintpdf.FileIO.deleteTempFile
import org.hammel.paintpdf.FileIO.fileType
import org.hammel.paintpdf.FileIO.filename
import org.hammel.paintpdf.FileIO.openTemporaryPictureFile
import org.hammel.paintpdf.FileIO.checkForTemporaryFile
import org.hammel.paintpdf.FileIO.saveFileFromUri
import org.hammel.paintpdf.FileIO.saveTemporaryPictureFile
...
import org.hammel.paintpdf.common.CREATE_FILE_DEFAULT
import org.hammel.paintpdf.common.LOAD_IMAGE_CATROID
import org.hammel.paintpdf.common.LOAD_IMAGE_DEFAULT
import org.hammel.paintpdf.common.LOAD_IMAGE_IMPORT_PNG
import org.hammel.paintpdf.common.MainActivityConstants.ActivityRequestCode
import org.hammel.paintpdf.common.MainActivityConstants.CreateFileRequestCode
import org.hammel.paintpdf.common.MainActivityConstants.LoadImageRequestCode
import org.hammel.paintpdf.common.MainActivityConstants.PermissionRequestCode
import org.hammel.paintpdf.common.MainActivityConstants.SaveImageRequestCode
import org.hammel.paintpdf.common.PERMISSION_EXTERNAL_STORAGE_SAVE
import org.hammel.paintpdf.common.PERMISSION_EXTERNAL_STORAGE_SAVE_CONFIRMED_FINISH
import org.hammel.paintpdf.common.PERMISSION_EXTERNAL_STORAGE_SAVE_CONFIRMED_LOAD_NEW
import org.hammel.paintpdf.common.PERMISSION_EXTERNAL_STORAGE_SAVE_CONFIRMED_NEW_EMPTY
import org.hammel.paintpdf.common.PERMISSION_EXTERNAL_STORAGE_SAVE_COPY
import org.hammel.paintpdf.common.PERMISSION_REQUEST_CODE_IMPORT_PICTURE
import org.hammel.paintpdf.common.PERMISSION_REQUEST_CODE_REPLACE_PICTURE
import org.hammel.paintpdf.common.REQUEST_CODE_IMPORT_PNG
import org.hammel.paintpdf.common.REQUEST_CODE_INTRO
import org.hammel.paintpdf.common.REQUEST_CODE_LOAD_PICTURE
import org.hammel.paintpdf.common.RESULT_INTRO_MW_NOT_SUPPORTED
import org.hammel.paintpdf.common.SAVE_IMAGE_DEFAULT
import org.hammel.paintpdf.common.SAVE_IMAGE_FINISH
import org.hammel.paintpdf.common.SAVE_IMAGE_LOAD_NEW
import org.hammel.paintpdf.common.SAVE_IMAGE_NEW_EMPTY
import org.hammel.paintpdf.common.TEMP_PICTURE_NAME
...
import org.hammel.paintpdf.controller.ToolController
import org.hammel.paintpdf.dialog.PermissionInfoDialog
import org.hammel.paintpdf.iotasks.BitmapReturnValue
...
import org.hammel.paintpdf.tools.implementation.CLICK_TIMEOUT_MILLIS
import org.hammel.paintpdf.tools.implementation.CONSTANT_3
...
open class MainActivityPresenter(
    ...
    private val view: MainActivityContracts.MainView,
    ...
    override fun onNewImage() {
        ...
        fileType = FileIO.FileType.PNG
        deleteTempFile(internalMemoryPath)
    ...
    override fun switchBetweenVersions(@PermissionRequestCode requestCode: Int, isExport: Boolean) {
        ...
        if (model.isOpenedFromCatroid) {
            storeImageUri = model.savedPictureUri
        }
    ...
    override fun handleActivityResult(
        ...
        when (requestCode) {
            ...
            REQUEST_CODE_INTRO -> if (resultCode == RESULT_INTRO_MW_NOT_SUPPORTED) {
                navigator.showToast(
                    R.string.pdfPaint_intro_split_screen_not_supported,
                    Toast.LENGTH_LONG
                )
            }
    ...
    override fun onLoadImagePostExecute(
        ...
        if (result.model != null) {
            ...
            fileType = FileIO.FileType.CATROBAT
            if (uri != null) {
                val name = getFileName(uri)
                if (name != null) {
                    filename = name.substring(0, name.length - fileType.toExtension().length)
                }
            }
            return
        }
                ...
                if (uri != null) {
                    ...
                    if (name != null) {
                        if (name.endsWith(FileIO.FileType.JPG.value) || name.endsWith("jpeg")) {
                            ...
                            fileType = FileIO.FileType.JPG
                        } else if (name.endsWith(FileIO.FileType.PNG.value)) {
                            compressFormat = Bitmap.CompressFormat.PNG
                            fileType = FileIO.FileType.PNG
                        } else {
                            fileType = FileIO.FileType.ORA
                        }
                        filename = name.substring(0, name.length - fileType.toExtension().length)
                    }
                }
    ...
    override fun saveNewTemporaryImage() {
        saveTemporaryPictureFile(internalMemoryPath, commandSerializer)
    }

    override fun openTemporaryFile(): WorkspaceReturnValue? =
        openTemporaryPictureFile(commandSerializer)

    override fun checkForTemporaryFile(): Boolean =
        checkForTemporaryFile(internalMemoryPath)
    ...
    companion object {
        ...
        private fun getDataColumn(
            ...
            try {
                ...
                saveFileFromUri(uri, file, context)
