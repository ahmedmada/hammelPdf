...
import org.hammel.paintpdf.FileIO.saveBitmapToFile
import org.hammel.paintpdf.FileIO.saveBitmapToUri
...
class SaveImage(
    ...
    private fun getImageUri(
        ...
        return if (uri == null) {
            val imageUri = saveBitmapToFile(filename, bitmap, callback.contentResolver, context)
            imageUri
        } else {
            uri?.let { saveBitmapToUri(it, bitmap, context) }
        }
    }
    ...
    fun execute() {
        ...
        scopeIO.launch {
            try {
                ...
                val filename = defaultFileName
                currentUri = if (fileType == FileIO.FileType.ORA) {
                ...
                } else if (fileType == FileIO.FileType.CATROBAT) {
                    if (uri != null) {
                        uri?.let {
